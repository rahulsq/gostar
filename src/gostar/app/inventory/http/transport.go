...
import (
	...
	"errors"
	"fmt"
	endpoint "gostar/app/inventory/endpoint"
...
func MakeHandler(inv service.Service, logger kitlog.Logger) http.Handler {
	...
	getInventoryHandler := kithttp.NewServer(
		endpoint.MakeGetInventoryEndpoint(inv),
...
func decodeGetInventoryRequest(_ context.Context, r *http.Request) (interface{}, error) {
	...
	return endpoint.GetInventoryRequest{
		ProductId: productId,
	}, nil
}
...
func encodeError(_ context.Context, err error, w http.ResponseWriter) {
	w.Header().Set("Content-Type", "application/json; charset=utf-8")
	switch err {
	case service.ErrInvalidArgument, service.RecordNotFound:
