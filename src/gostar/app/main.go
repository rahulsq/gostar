...
import (
	...
	invhttp "gostar/app/inventory/http"
	invmiddle "gostar/app/inventory/middleware"
	invsvc "gostar/app/inventory/service"
	orderendpoint "gostar/app/order/endpoint"
	ordergrpc "gostar/app/order/grpc"
	orderhttp "gostar/app/order/http"
	rep "gostar/app/order/implementation"
	ordersvc "gostar/app/order/service"
	serv "gostar/app/pkg/service"

	addpb "gostar/app/order/pb"
...
func main() {
	...
	var inv invsvc.Service
	inv = invsvc.NewService(db)
	inv = invmiddle.NewLoggingService(log.With(logger, "component", "inventory"), inv)
	inv = invmiddle.NewInstrumentingService(
	...
	orderService := ordersvc.NewService(repo, logger)

	httpLogger := log.With(logger, "component", "http")

	mux := http.NewServeMux()

	mux.Handle("/inventory/", invhttp.MakeHandler(inv, httpLogger))
	mux.Handle("/order/", orderhttp.MakeHandler(orderService, httpLogger))
	...
	var g group.Group
	var (
		endpoints  = orderendpoint.MakeEndpoint(orderService)
		grpcServer = ordergrpc.NewGRPCServer(endpoints, logger)
	)
